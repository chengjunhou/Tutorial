...
除此之外我还考虑加入游戏时长这一信息，虽然这个信息在比赛开始之前并不能获知，但 Dota2 里英雄的作用随着游戏时长增加有着巨大的变化。
...
另外，我还想过把1条数据变成2条来建模，即自变量为己方阵容，因变量为己方是否获胜。
...
此时树模型的特点是低复杂度（`max.depth=2`），大数量（`nrounds=256`）：
...
```
```{r, message=FALSE, warning=FALSE, echo=FALSE}
trY <- readRDS("rds/trY1")
ooY <- readRDS("rds/ooY1")
roc_trY <- prediction(trY, mrd[,232])
roc_ooY <- prediction(ooY, vrd[,232])
c(train=performance(roc_trY, measure="auc")@y.values[[1]],
  holdout=performance(roc_ooY, measure= "auc")@y.values[[1]])
...
例如宙斯和力丸搭配对赢得比赛有正面作用，但如果队伍中同时出现后期才强势的英雄如幽鬼和敌法师那基本就别想赢了。
不过胜率还是可以说明一些问题的，比如说天辉夜魇平均胜率分别为53.6%和46.4%，
但选了幽鬼（编号67）之后胜率有明显提升达到62.3%和55.7%，选齐天大圣（编号114）则只有46.2%和39.6%。
...
关于回归模型可以参考我之前的文章，[链接](http://cos.name/2016/10/data-mining-1-lasso/)。
...
```
```{r, message=FALSE, warning=FALSE, echo=FALSE}
trY <- readRDS("rds/trY2")
ooY <- readRDS("rds/ooY2")
roc_trY <- prediction(trY, mrd[,232])
roc_ooY <- prediction(ooY, vrd[,232])
c(train=performance(roc_trY, measure="auc")@y.values[[1]],
  holdout=performance(roc_ooY, measure= "auc")@y.values[[1]])
...
加入游戏时长这个变量后，模型 AUC 又有了明显提升，与**数据处理**章节预想得一样。
...
这里敌法师，祈求者，斯拉克跃升到变量重要度排行的前列，他们都是比较典型胜率随游戏时长增加而显著提高的英雄。
